...
export default function Create() {
  ...
  async function test(e) {
    ...
    try {
      await axios.post("http://localhost:5000/app", task);
      navigate("/");
        ...
        <div className='fixed left-1 md:left-0 flex flex-col  w-full justify-center  text-black  items-center'>
          {er ? (
            <div role='alert' className=' md:w-[40%] w-[80%]'>
              <div class='mt-[20px] w-[100%] bg-red-500 text-white font-bold rounded-t px-4 py-2'>
                Danger
              </div>
          ...
          <h1 className='mt-[40px]  text-3xl font-bold  '>Create Task</h1>
          <form
          ...
          >
            ...
            <button
              type='submit'
              className='transition ease-in-out delay-150 hover:-translate-y-1 hover:scale-95 hover:bg-blue-400 duration-300  text-center bg-blue-500  text-white rounded-md p-2 cursor-pointer font-bold'
            >
              add
            </button>
          </form>
